...
function updateZoneFile($IP){
	...
	system("/usr/bin/sudo /usr/sbin/rndc reload ".$CFG["ORIGIN"]);
}
...
function updateHostIPFile($hostName,$timestamp){
	...
	$IPDB["records"][$hostName]["timestamp"]=time();					// update time stamp irrespective of ip change to detect stale ip addresses.
